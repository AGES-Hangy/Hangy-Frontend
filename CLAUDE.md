...
- `src/app/index.tsx` is the root route (`/`); it only does `<Redirect href="/Splash" />` — it must not render its own content. `src/app/Splash/index.tsx` checks the saved session token and sends the user to `/Home` (token present) or `/Login` (no token). After a successful login, `src/app/Login/index.tsx` also sends the user to `/Home`, the first tab.
