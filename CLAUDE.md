...
- `(tabs)/_layout.tsx` renders `src/components/TopAppBar` as every screen's header: tab roots get `variant="Home"` (avatar slot, Hangy logo, notifications bell linking to `(screens)/Notifications`), and `(screens)/` routes get `variant="Detail"` (back button + title derived from the folder name). The `Tabs` navigator has `backBehavior="history"` so `router.canGoBack()`/`router.back()` reflect the actual screens visited (not just a fixed first tab) — don't remove that prop, the back button silently breaks without it.
